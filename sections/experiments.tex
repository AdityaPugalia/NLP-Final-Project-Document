...
filtered from FineWeb \parencite{penedo2024fineweb}.
...
Byte-Pair Encoder with byte fallback (i.e. rare characters are encoded in raw UTF-8 instead of a generic \texttt{<UNK>} token), and tokenised sequences are loaded via a distributed data loader that streams directly from the shards during training.
...
We use three complementary metrics to assess both model performance and efficiency.
...
\begin{equation*}
    \mathcal{L}_{\text{bpb}} = \frac{\sum_{i=1}^{N} \ell_i \cdot \mathbb{I}[b_i > 0]}
                                    {\sum_{i=1}^{N} |b_i|}
\end{equation*}
...
The NanoChat leaderboard record achieves a CORE score of 0.2602 in 2.76 hours.
%on an 8 $\times$ H100 node; the reference GPT-2 checkpoint achieves 0.2565.
